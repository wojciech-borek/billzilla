...
## 2. Routing widoku
- Modal dostępny zarówno na `/dashboard`, jak i na `/groups/:id` (podgląd grupy).
- Otwieranie modala sterowane parametrem URL: `?modal=create-group`.
  - Wejście na `/dashboard?modal=create-group` lub `/groups/:id?modal=create-group` otwiera modal.
...
- Stan modala: synchronizacja z URL param `modal=create-group` na `/dashboard` i `/groups/:id`; alternatywnie lokalny stan (fallback bez deep-linku).
...
- Otwórz modal: klik przycisku „Utwórz grupę” na `/dashboard` lub w podglądzie grupy (`/groups/:id`). Oba przypadki ustawiają `?modal=create-group` w URL.
...
6. Zepnij modal ze stanem URL `?modal=create-group` na `/dashboard` i `/groups/:id` (otwarcie/ zamknięcie modyfikuje URL przez push/replace).
