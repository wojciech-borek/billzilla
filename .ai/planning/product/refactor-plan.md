...
- `src/__tests__/services/testHelpers.ts` (linia 12)
- `src/__tests__/testTypes.ts` (linia 3)
- `src/__tests__/services/authService.test.ts` (linia 2)
- `src/__tests__/services/passwordResetService.test.ts` (linia 5)
- `src/lib/hooks/useSupabaseAuth.ts` (linia 3) ✅ Poprawne użycie, ale można uprościć

#### Rozwiązanie

```typescript
// PRZED (testHelpers.ts):
import type { AuthError } from "@supabase/supabase-js";

// PO:
import type { SupabaseClient } from "@/db/supabase.client";
// Dla AuthError można zaimportować z głównego pliku types
...
export type { AuthError, User, Session } from "@supabase/supabase-js";
```
...
Walidacja audio file występuje w dwóch miejscach:
...
Przenieść walidację do dedykowanego modułu:

```typescript
// src/lib/utils/audioValidation.ts
export class AudioFileValidator {
  ...
  static validate(file: File | Blob): { valid: boolean; error?: string } {
    // Check size
    ...
    if (file.type && !this.isSupportedFormat(file.type)) {
      return {
        valid: false,
        error: `Unsupported format: ${file.type}. Supported: ${this.SUPPORTED_FORMATS.join(", ")}`,
      };
    }
...
if (!validation.valid) {
  return new Response(
    JSON.stringify({
      error: { code: "INVALID_AUDIO", message: validation.error },
    }),
    { status: 400 }
  );
}
...
### 🟡 4. COMPLEX FUNCTIONS - POTRZEBA REFAKTORYZACJI
...
Niektóre funkcje są zbyt długie:

- `src/lib/services/aiReviewService.ts`: `performPullRequestReview` (100+ linii)
- `src/lib/services/expenseService.ts`: `ExpenseUnitOfWork.execute` (80+ linii)
...
private async validateAndPrepare(): Promise<void> {
  const groupData = await this.validateGroupMembership();
  await this.validateParticipants(groupData);
}

private async persistExpenseData(): Promise<void> {
  await this.createExpense();
  await this.createExpenseSplits();
}
