...
export function useGroupsList({
  status = 'active',
  limit = 20,
}: UseGroupsListParams = {}): GroupsQueryState & { 
  ...
  const fetchGroups = useCallback(async (resetData: boolean = true, currentOffset: number = 0) => {
    setState((prev) => ({ 
      ...prev, 
      loading: true, 
      error: null,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error?.message || `Failed to fetch groups: ${response.status}`
        );
      }
