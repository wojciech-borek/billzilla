...
export const SignupForm = memo(function SignupForm({ successMessage, errorMessage }: SignupFormProps) {
  ...
  return (
    <div className="space-y-6">
      {" "}
      {/* Status messages */} {successMessage && <StatusMessage type="info" message={successMessage} />}{" "}
      {isSuccess && (
        <StatusMessage
          type="success"
      ...
      {signupError && <StatusMessage type="error" message={signupError} />} {/* Registration form */}{" "}
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          ...
          {isLoading ? (
            <>
              {" "}
              <div className="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> Trwa
              rejestracja...{" "}
            </>
          ) : (
            "Zarejestruj się"
